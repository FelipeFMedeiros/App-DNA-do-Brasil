...
function fixedFooterSpace() {
    if ($(".adminuiux-sidebar").length <= 0) {
        $(".adminuiux-mobile-footer").css('margin-left', '0')
    }
...
function scrolldirection() {
    ...
    var pagelength = document.documentElement.scrollHeight - 50;

    window.addEventListener("scroll", function () {
        var st = document.documentElement.scrollTop;
        //alert(document.documentElement.scrollTop +"/"+pagelength)
        ...
        /* fixed buttons space */
        if ($(".adminuiux-mobile-footer").length > 0 && $("body").hasClass('scrollup') === true) {
            $("#fixedbuttons").css("padding-bottom", $(".adminuiux-mobile-footer").outerHeight());
        } else {
            $("#fixedbuttons").css("padding-bottom", 0);
        }
    }, false);
}
...
function swipergallerythumbnav() {
    if ($('.swiperthumb').length > 0) {
        var swiper = new Swiper(".swiperthumbnav", {
            spaceBetween: 16,
            slidesPerView: 'auto',
            freeMode: true,
          ...
          var swiper2 = new Swiper(".swiperthumb", {
            spaceBetween: 0,
            thumbs: {
              swiper: swiper,
            },
...
function increamenter(){
    if($('.increamenter').length > 0 ){
       ...
       $('.increamenter').each(function(){
        var elIncreamenter = $(this);
        var elValue = $(this).find('.increamenter-value');
        var value = parseInt(elValue.val());

        /* add  */
        elIncreamenter.find('.increamenter-add').on('click', function(e){
            e.preventDefault();
            value = parseInt(elValue.val());
            elValue.val(value + 1);
            return false;
        });    
        
        /* remove */        
        elIncreamenter.find('.increamenter-remove').on('click', function(e){
            e.preventDefault();             
            value = parseInt(elValue.val());

            if(value > 0 ){
                elValue.val(value - 1);
                elIncreamenter.find('.increamenter-remove').attr('disabled', false);                
                return false; 
            }else{
                elIncreamenter.find('.increamenter-remove').attr('disabled', true);
            }    
        });
